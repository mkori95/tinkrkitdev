...
export default function SvgToPngPage() {
  ...
  const convert = useCallback(() => {
    ...
    try {
      // Inject width/height into SVG for correct rendering
      const parser = new DOMParser();
      const doc = parser.parseFromString(svg, "image/svg+xml");
      ...
      img.onload = () => {
        ...
        // PNG supports transparency — no fill needed
  ...
  return (
    <ToolLayout
      ...
      howToSteps={[
        ...
        "Set your desired output width and height, then click Convert to PNG",
    ...
    >
      <div className="space-y-4">
        ...
        {/* SVG text area (shown after file load OR as primary input) */}
        <div>
        ...
        {/* Dimension inputs + convert button */}
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="w-28 rounded-lg border border-border bg-card px-3 py-2 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
            />
          </div>
          <div className="flex flex-col gap-1">
        ...
        {status === "done" && outputUrl && (
          <div className="space-y-3 rounded-xl border border-border bg-card p-4">
            {/* Preview */}
            <div className="flex items-center justify-center overflow-hidden rounded-lg bg-[repeating-conic-gradient(#ccc_0%_25%,transparent_0%_50%)] bg-[length:16px_16px] dark:bg-[repeating-conic-gradient(#333_0%_25%,transparent_0%_50%)]" style={{ minHeight: 120 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={outputUrl} alt="PNG output" className="max-h-64 max-w-full object-contain" />
            </div>

            {/* Info bar + actions */}
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-xs text-muted-foreground">
                PNG · {width} × {height} px · {fmtSize(outputSize)}
              </p>
