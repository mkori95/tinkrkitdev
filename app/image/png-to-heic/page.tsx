...
const tool = TOOLS.find((t) => t.slug === "png-to-heic")!;
...
export default function PngToHeicPage() {
  ...
  function convert(f: File, q: QualityValue) {
    ...
    img.onload = async () => {
      ...
      // PNG may have transparency — fill white background for HEIC (no alpha)
      ...
      <div className="space-y-4">
        ...
        {!file ? (
          ...
          >
            ...
            <div>
              <p className="text-sm font-medium">Drop a PNG image here or click to upload</p>
              <p className="mt-1 text-xs text-muted-foreground">PNG — max 20 MB · Transparency replaced with white</p>
            </div>
            <input
              ref={inputRef}
              type="file"
          ...
          <div className="space-y-4">
            ...
            {originalUrl && (
              <div className="space-y-2">
                <label className="text-sm font-medium">Original (PNG)</label>
                <div className="flex max-h-64 items-center justify-center overflow-hidden rounded-lg border border-border bg-muted/20 p-2">
                  ...
                  <img src={originalUrl} alt="Original PNG" className="max-h-full max-w-full object-contain" />
                </div>
              </div>
            )}
            ...
            {status === "unsupported" && (
              <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-4 space-y-2">
                ...
                <p className="text-sm text-amber-700/80 dark:text-amber-300/80">
                  Your current browser doesn&apos;t support native HEIC encoding. To convert PNG to HEIC:
                </p>
                <ul className="text-sm text-amber-700/80 dark:text-amber-300/80 list-disc list-inside space-y-1">
                  ...
                  <li>Use Apple&apos;s <strong>Preview app</strong> on macOS → File → Export → HEIC</li>
                  <li>Use <strong>Photos app</strong> on macOS — import then export as HEIC</li>
                </ul>
              </div>
            )}
          </div>
        ...
        <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
          <p className="font-medium text-foreground mb-1">PNG transparency note</p>
          <p>HEIC does not support transparency (alpha channel). Any transparent areas in your PNG will be replaced with a white background. If your image has a transparent background that needs to be preserved, consider keeping it as PNG instead.</p>
        </div>
      </div>
    </ToolLayout>
  );
