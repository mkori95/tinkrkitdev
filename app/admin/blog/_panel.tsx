...
function PreviewModal({ postId, onClose }: { postId: string; onClose: () => void }) {
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div>
              ...
              {post.description && <p className="mb-4 text-sm text-muted-foreground">{post.description}</p>}
...
function PostCard({
  post, onPreview, onApprove, onReject, onUnpublish, onReconsider, busy,
}: {
  post:          Post;
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {onApprove && (
          <button onClick={() => onApprove(post.id)} disabled={busy}
        ...
        {onUnpublish && (
          <button onClick={() => onUnpublish(post.id)} disabled={busy}
        ...
        {onReconsider && (
          <button onClick={() => onReconsider(post.id)} disabled={busy}
...
// ── Main panel (exported — rendered by the server page wrapper) ───────────────
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 border-b border-border bg-background/90 backdrop-blur-sm">
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-3">
            <Logo size="sm" />
            <span className="hidden text-sm font-medium text-muted-foreground sm:block">/ Blog Admin</span>
          </div>
          <button
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Review, approve, and manage submitted blog posts.</p>
        </div>
      ...
      {previewId && <PreviewModal postId={previewId} onClose={() => setPreviewId(null)} />}
      {rejectPost && (
        <RejectModal
          post={rejectPost}
